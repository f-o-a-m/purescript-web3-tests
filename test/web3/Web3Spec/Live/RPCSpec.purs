...
import Network.Ethereum.Web3 (Block(..), ChainCursor(..), TransactionReceipt(..), _from, _to, _value, convert, defaultTransactionOptions, fromMinorUnit, httpProvider, mkHexString, runWeb3)
...
import Web3Spec.Live.Utils (assertWeb3, nodeUrl, pollTransactionReceipt)

spec :: SpecT Aff Unit Aff Unit
spec =
  beforeAll (liftEffect $ httpProvider nodeUrl) $ do
    describe "It should be able to test all the web3 endpoints live" do
      it "Can get the network version" \provider -> do
        eRes <- runWeb3 provider $ Api.net_version
        eRes `shouldSatisfy` isRight
      it "Can call net_listening" \provider -> do
        eRes <- runWeb3 provider $ Api.net_listening
        eRes `shouldSatisfy` isRight
      it "Can call net_getPeerCount" \provider -> do
        eRes <- runWeb3 provider $ Api.net_getPeerCount
        eRes `shouldSatisfy` isRight
      it "Can call eth_protocolVersion" \provider -> do
        eRes <- runWeb3 provider $ Api.eth_protocolVersion
        eRes `shouldSatisfy` isRight
      it "Can call eth_getSyncing" \provider -> do
        eRes <- runWeb3 provider $ Api.eth_getSyncing
        eRes `shouldSatisfy` isRight
      it "Can call eth_coinbase" \provider -> do
        eRes <- runWeb3 provider $ Api.eth_coinbase
        eRes `shouldSatisfy` isRight
      it "Can call eth_mining" \provider -> do
        eRes <- runWeb3 provider $ Api.eth_mining
        eRes `shouldSatisfy` isRight
      it "Can call eth_hashrate" \provider -> do
        eRes <- runWeb3 provider $ Api.eth_hashrate
        eRes `shouldSatisfy` isRight
      it "Can call eth_blockNumber" \provider -> do
        eRes <- runWeb3 provider $ Api.eth_blockNumber
        eRes `shouldSatisfy` isRight
      it "Can call eth_accounts and eth_getBalance" \provider -> do
        eRes <-
          runWeb3 provider
            $ do
        ...
        eRes `shouldSatisfy` isRight
      it "Can call eth_getTransactionCount" \provider -> do
        eRes <-
          runWeb3 provider do
            accounts <- Api.eth_getAccounts
            Api.eth_getTransactionCount (unsafePartial fromJust $ accounts !! 0) Latest
        eRes `shouldSatisfy` isRight
      it "Can call eth_getBlockByNumber, eth_getBlockTransactionCountByHash, getBlockTransactionCountByNumber" \provider -> do
        Tuple count1 count2 <-
      ...
      it "Can call eth_getUncleCountByBlockHash eth_getUncleCountByBlockNumber" \provider -> do
        Tuple count1 count2 <-
          assertWeb3 provider do
            bn <- Api.eth_blockNumber
            Block block <- Api.eth_getBlockByNumber (BN bn)
            ...
            pure $ Tuple count1 count2
        count1 `shouldEqual` count2
      it "Can call eth_getBlockByHash" \provider -> do
        eRes <-
          runWeb3 provider do
            bn <- Api.eth_blockNumber
            Block block <- Api.eth_getBlockByNumber (BN bn)
      ...
      it "Can call personal_sign, personal_ecRecover, and they should coincide with eth-core" \provider -> do
        let
      ...
      it "Can call eth_estimateGas" \provider -> do
        eRes <- runWeb3 provider $ Api.eth_estimateGas (defaultTransactionOptions # _value %~ map convert)
        eRes `shouldSatisfy` isRight
      it "Can call eth_getTransactionByBlockHashAndIndex eth_getBlockTransactionByBlockNumberAndIndex" \provider -> do
        txHash <-
          assertWeb3 provider do
            accounts <- Api.eth_getAccounts
            let
